...
namespace EPOS_API.Controllers
{
    public class KDSController : Controller
    {
        ...
        public KDSController(IConfiguration config)
        {
            _config = config;
        }

        [HttpPost("KDS")]
        public string Kds([FromBody] EPOS_API.Model.KDSModel obj)
        {
            var response = KitchenDisplaySystem(obj, HttpContext);
            string json = JsonConvert.SerializeObject(response, Formatting.Indented);
            return json;
        }
        private dynamic KitchenDisplaySystem(EPOS_API.Model.KDSModel obj, HttpContext context)
        {
            try
            {
                if (Convert.ToBoolean(context.Items["Validate"]) == true)
                {
                    ...
                    parm.Add(new SqlParameter() { ParameterName = "@OrderDetailLogStr", SqlDbType = SqlDbType.NVarChar, Value = obj.OrderDetailLogStr });
                    var spName = "SP_KDS";
