...
namespace EPOS_API.Controllers
{
    public class RequisitionController : Controller
    {
        ...
        public RequisitionController(IConfiguration config)
        {
            _config = config;
        }
        [HttpPost("CrudRequisition")]
        public string CrudRequisition([FromBody] EPOS_API.Model.RequisitionModel obj)
        {
            var response = Crud_Requisition(obj, HttpContext);
            string json = JsonConvert.SerializeObject(response, Formatting.Indented);
            return json;
        }
        private dynamic Crud_Requisition(EPOS_API.Model.RequisitionModel obj, HttpContext context)
        {
            try
            {
                if (Convert.ToBoolean(context.Items["Validate"]) == true)
                {
                    ...
                    parm.Add(new SqlParameter() { ParameterName = "@RequisitionDetail", SqlDbType = SqlDbType.Structured, Value = obj.RequisitionDetail.Count == 0 ? null : CommonObjects.ToDataTable(obj.RequisitionDetail.AsEnumerable().ToList()) });
                    parm.Add(new SqlParameter() { ParameterName = "@Date", SqlDbType = SqlDbType.NVarChar, Value = obj.Date });
                    parm.Add(new SqlParameter() { ParameterName = "@RequisitionNumber", SqlDbType = SqlDbType.NVarChar, Value = obj.RequisitionNumber });


                    var spName = "SP_Requisition";
