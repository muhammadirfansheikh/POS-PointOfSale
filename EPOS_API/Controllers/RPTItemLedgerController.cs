...
{
    public class RPTItemLedgerController : Controller
    {
        private IConfiguration _config;
        MessageDate<dynamic> responseDetail = new MessageDate<dynamic>();
        public RPTItemLedgerController(IConfiguration config)
        {
            _config = config;
        }
        [HttpPost("CrudRPTItemLedger")]
        public string CrudRPTItemLedger([FromBody] EPOS_API.Model.RPTItemLedgerModel obj)
        {
            var response = Crud_RPTItemLedger(obj, HttpContext);
            string json = JsonConvert.SerializeObject(response, Formatting.Indented);
            return json;
        }
        private dynamic Crud_RPTItemLedger(EPOS_API.Model.RPTItemLedgerModel obj, HttpContext context)
        {
            try
            {
