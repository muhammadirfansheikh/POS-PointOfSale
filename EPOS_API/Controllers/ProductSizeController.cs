...
namespace EPOS_API.Controllers
{
    public class ProductSizeController : Controller
    {
        ...
        public ProductSizeController(IConfiguration config)
        {
            _config = config;
        }
        [HttpPost("CrudProductSize")]
        public string CrudProductSize([FromBody] EPOS_API.Model.ProductSizeModel obj)
        {
            var response = Crud_Prodcut_Size(obj, HttpContext);
            string json = JsonConvert.SerializeObject(response, Formatting.Indented);
            return json;
        }
        private dynamic Crud_Prodcut_Size(EPOS_API.Model.ProductSizeModel obj, HttpContext context)
        {
            try
            {
                if (Convert.ToBoolean(context.Items["Validate"]) == true)
                {
                    ...
                    parm.Add(new SqlParameter() { ParameterName = "@SizeName", SqlDbType = SqlDbType.NVarChar, Value = obj.SizeName });
