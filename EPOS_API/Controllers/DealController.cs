...
namespace EPOS_API.Controllers
{
    public class DealController : Controller
    {
        ...
        public DealController(IConfiguration config)
        {
            _config = config;
        }
        [HttpPost("CrudDeal")]
        public string CrudDeal([FromBody] EPOS_API.Model.DealModel obj)
        {
            var response = CrudDeal(obj, HttpContext);
            string json = JsonConvert.SerializeObject(response, Formatting.Indented);
            return json;
        }
        private dynamic CrudDeal(EPOS_API.Model.DealModel obj, HttpContext context)
        {
            try
            {
                if (Convert.ToBoolean(context.Items["Validate"]) == true)
                {
                    ...
                    parm.Add(new SqlParameter() { ParameterName = "@tblDealDescription", SqlDbType = SqlDbType.Structured, Value = CommonObjects.ToDataTable(obj.tblDealDescription.AsEnumerable().ToList()) });
                    parm.Add(new SqlParameter() { ParameterName = "@tblDealItemDetail", SqlDbType = SqlDbType.Structured, Value = CommonObjects.ToDataTable(obj.tblDealItemDetail.AsEnumerable().ToList()) });
                    parm.Add(new SqlParameter() { ParameterName = "@CategoryId", SqlDbType = SqlDbType.Int, Value = obj.CategoryId });
                    parm.Add(new SqlParameter() { ParameterName = "@DealName", SqlDbType = SqlDbType.NVarChar, Value = obj.DealName });
                    parm.Add(new SqlParameter() { ParameterName = "@UserId", SqlDbType = SqlDbType.Int, Value = obj.UserId });

                    var spName = "SP_CrudDeal";
