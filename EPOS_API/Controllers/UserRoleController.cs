...
namespace EPOS_API.Controllers
{
    public class UserRoleController : Controller
    {
        ...
        [HttpPost("CrudUserRole")]
        public string CrudUserRole([FromBody] EPOS_API.Model.UserRoleModel obj)
        {
            var response = CrudUserRole(obj, HttpContext);
            string json = JsonConvert.SerializeObject(response, Formatting.Indented);
            return json;
        }
        private dynamic CrudUserRole(EPOS_API.Model.UserRoleModel obj, HttpContext context)
        {
            try
            {
                if (Convert.ToBoolean(context.Items["Validate"]) == true)
                {
                    ...
                    parm.Add(new SqlParameter() { ParameterName = "@RoleName", SqlDbType = SqlDbType.NVarChar, Value = obj.RoleName });
