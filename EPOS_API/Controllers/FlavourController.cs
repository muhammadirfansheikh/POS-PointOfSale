...
namespace EPOS_API.Controllers
{
    public class FlavourController : Controller
    {
        ...
        public FlavourController(IConfiguration config)
        {
            _config = config;
        }
        [HttpPost("CrudFlavour")]
        public string CrudFlavour([FromBody] EPOS_API.Model.FlavourModel obj)
        {
            var response = Crud_Flavour(obj, HttpContext);
            string json = JsonConvert.SerializeObject(response, Formatting.Indented);
            return json;
        }
        private dynamic Crud_Flavour(EPOS_API.Model.FlavourModel obj, HttpContext context)
        {
            try
            {
                if (Convert.ToBoolean(context.Items["Validate"]) == true)
                {
                    ...
                    parm.Add(new SqlParameter() { ParameterName = "@FlavourId", SqlDbType = SqlDbType.Int, Value = obj.FlavourId });
                    parm.Add(new SqlParameter() { ParameterName = "@FlavourName", SqlDbType = SqlDbType.NVarChar, Value = obj.FlavourName });
